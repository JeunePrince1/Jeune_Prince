...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '2290152011653@s.whatsapp.net'; // ton num茅ro ou le groupe cible
...
鈺斺晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晽
      *Jeun_Prince Bot Connected Successfully* 馃殌
...
*Jeune_Prince 229*
                ...
                await sock.sendMessage(chatId, {
                    image: { url: imagePath },
                    caption: messageText,
                    footer: '馃捇 Powered by Jeune_Prince',
                });
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            console.log('鈿狅笍 Not logged in. Preparing pairing process...');
            try {
                const asPremium = true; // await deployAsPremium();
                const number = 2290152011653; // mettez votre num茅ro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '2290152011653' };
                    configmanager.saveP();
                    configmanager.premiums.premiumUser['p'] = { premium: number };
                    configmanager.saveP();
                }

                console.log(`馃攧 Requesting pairing code for ${number}`);
                const code = await sock.requestPairingCode(number, 'KINGSON1);
                console.log('馃摬 Pairing Code:', code);
                console.log('馃憠 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['2290152011653'], // emplace par ton num茅ro WhatsApp 
